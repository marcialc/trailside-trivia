    ...
    <>
      ...
      >
        ...
        <div className={styles.head}>
          ...
          <button ref={closeRef} className={styles.closebtn} aria-label={ui.close} onClick={onClose}>
            ×
          </button>
          <div className={styles.eyebrow}>
            {shown ? `${tt(shown.region)}  ·  ${tt(shown.coord)}` : ''}
          </div>
          <h2 className={styles.title} id="sheetTitle">
            {shown ? tt(shown.name) : ''}
          </h2>
          <p className={styles.sub}>{shown ? tt(shown.teaser) : ''}</p>
        </div>
          ...
          {shown?.facts.map((f, i) => (
            <div className={styles.fact} key={i}>
              ...
              <div className={styles.factBody}>
                <span className={`${styles.tag} ${styles.soft}`}>{tt(f.tag)}</span>
                <div className={styles.factText}>
                  {highlight(tt(f.text), term, (chunk, key) => (
